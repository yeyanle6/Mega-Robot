...
class AdvancedPCDConverter:
    """高级PCD转换器，专门处理MID360激光雷达数据"""
    ...
    def detect_ground_advanced(self, points, method='histogram'):
        ...
        Args:
            ...
            method: 检测方法 ('histogram', 'clustering', 'ransac')
        ...
        if method == 'histogram':
            ...
            return self._detect_ground_ransac(points)
        else:
    ...
    def _detect_ground_clustering(self, points):
        ...
        # 使用DBSCAN聚类
        clustering = DBSCAN(eps=0.05, min_samples=100).fit(z_coords)
        labels = clustering.labels_
        
        # 找到最大的聚类（假设是地面）
        unique_labels, counts = np.unique(labels[labels >= 0], return_counts=True)
        if len(unique_labels) == 0:
            return self._detect_ground_histogram(points[:, 2])
        
        largest_cluster_label = unique_labels[np.argmax(counts)]
        ground_points = z_coords[labels == largest_cluster_label]
        
        return np.median(ground_points)
    
    def _detect_ground_ransac(self, points):
        """使用RANSAC方法检测地面平面"""
        # 使用RANSAC拟合平面
        ...
        if len(inliers) > 0:
            ...
            ground_height = np.median(inlier_points[:, 2])
            return ground_height
        else:
            return self._detect_ground_histogram(points[:, 2])
...
def main():
    parser = argparse.ArgumentParser(description='高级PCD点云文件转换为PGM栅格地图')
    ...
    parser.add_argument('--ground-method', choices=['histogram', 'clustering', 'ransac'], 
